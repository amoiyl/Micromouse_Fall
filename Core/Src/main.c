...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		...
		if(front_left_IR < threshold && front_right_IR < threshold) {
			isWallF = 1;
		} else if(left_IR < threshold) {
			isWallL = 1;
		} else if(right_IR < threshold) {
			isWallR = 1;
		}

		if(isWallF == 0) {
			move(1);
		} else if(isWallF == 1 && isWallR == 0) {
			turn(-1);
		} else {
			turn(1);
		}
